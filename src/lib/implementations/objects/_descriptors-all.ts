...
import {ExpandedPropertyDescriptor, TargetObject} from "../../abstract";
import {_properifyKey} from "./_properify-key";

export function _descriptorsMap(target: TargetObject, filter?: PropertyFilter): Map<PropertyKey, ExpandedPropertyDescriptor> {
    let descs = new Map<PropertyKey, ExpandedPropertyDescriptor>();
    let protos = filter.noInherited ? [target] : _protoChain(target, filter.stopAtProto);
...
export function _descriptorsAll(target: TargetObject, filter?: PropertyFilter) : ExpandedPropertyDescriptor[] {
